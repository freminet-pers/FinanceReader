...
The `2.23.6` entry is FinanceReader-specific. Entries from `2.22.0` downward are retained Feeder release history from the fork base; their upstream links and contributor attribution are intentionally preserved. Future FinanceReader entries should be added above that retained history rather than rewriting it. See [the project scope note](docs/PROJECT_SCOPE.md) for the maintenance boundary.
